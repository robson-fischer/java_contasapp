...
public class ContaRepository {
	...
	public void create(Conta conta) throws Exception {
		...
		Object[] params = { conta.getNome(), conta.getDescricao(), new java.sql.Date(conta.getData().getTime()),
				conta.getValor(), conta.getTipo(), conta.getUsuario().getId() };
	...
	public void update(Conta conta) throws Exception {
		...
		// criando os parametros que serão preenchidos no comando sql
		Object[] params = { conta.getNome(), conta.getDescricao(), new java.sql.Date(conta.getData().getTime()),
				conta.getValor(), conta.getTipo(), conta.getId() };
	...
	public List<Conta> findAll(Date dataMin, Date dataMax, Integer usuarioId) throws Exception {
		...
		String sql = "select * from conta " + "where data between ? and ? " + "and usuario_id = ? " + "order by data";

		Object[] params = { new java.sql.Date(dataMin.getTime()), new java.sql.Date(dataMax.getTime()), usuarioId };
